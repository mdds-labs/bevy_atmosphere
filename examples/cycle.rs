...
fn main() {
    App::new()
        ...
        .insert_resource(CycleTimer(Timer::new(
            core::time::Duration::from_millis(50), // Update our atmosphere every 50ms (in a real game, this would be much slower, but for the sake of an example we use a faster update)
            TimerMode::Repeating,
        )))
        .add_plugins((
            DefaultPlugins,
...
fn daylight_cycle(
    ...
    if timer.0.finished() {
        ...
        if let Ok((mut light_trans, mut directional)) = query.single_mut().into() {
